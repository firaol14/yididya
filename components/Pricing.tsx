...
const Pricing = () => {
  ...
  return (
    <section id="pricing" className="py-12 md:py-24 relative overflow-hidden bg-brand-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-3xl mx-auto text-center mb-10 md:mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-4 md:mb-6">{t.pricing.title}</h2>
          <p className="text-lg md:text-xl text-gray-400">{t.pricing.subtitle}</p>
        </div>

        <div className="grid md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto mb-12 md:mb-16 items-stretch">
            ...
            <div className="bg-[#0F0F0F] border border-brand-border rounded-3xl overflow-hidden opacity-60 relative flex flex-col grayscale-[0.5] hover:grayscale-0 transition-all duration-500 order-2 md:order-1">
                <div className="p-5 md:p-8 flex-grow">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <h3 className="text-xl md:text-2xl font-bold text-white">{t.pricing.groupCard.title}</h3>
                            <p className="text-gray-500 mt-1 text-sm">{t.pricing.groupCard.subtitle}</p>
                        </div>
                        <div className="bg-red-500/10 text-red-500 px-3 py-1 rounded-full text-[10px] md:text-xs font-bold tracking-wide uppercase border border-red-500/20">
                            {t.pricing.groupCard.status}
                        </div>
                    </div>
                     <div className="flex items-end gap-2 mb-6 md:mb-8">
                        <span className="text-3xl md:text-4xl font-extrabold text-white">{t.pricing.groupCard.waitlist}</span>
                    </div>
                    <button disabled className="w-full bg-brand-border/50 text-gray-500 cursor-not-allowed font-bold py-3 md:py-4 rounded-xl mb-6 md:mb-8 flex items-center justify-center gap-2 text-sm md:text-base">
                        ...
                        {t.pricing.groupCard.button}
                    </button>
                    <ul className="space-y-4 opacity-50">
                        {t.pricing.groupCard.features.map((feature, i) => (
                          <li key={i} className="flex items-center gap-3"><CheckCircle2 className="w-5 h-5 text-gray-600"/> <span className="text-gray-500 text-sm">{feature}</span></li>
                        ))}
                    </ul>
                </div>
            </div>
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-brand-green text-black font-bold px-4 py-1 rounded-b-lg text-[10px] md:text-xs uppercase tracking-wider z-20 shadow-lg whitespace-nowrap">
                {t.pricing.soloCard.badge}
            </div>

            <div className="p-5 md:p-10 flex-grow relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start mb-6 md:mb-8 gap-2 md:gap-0 mt-4 md:mt-0">
                    <div>
                        <h3 className="text-2xl md:text-3xl font-bold text-white">{t.pricing.soloCard.title}</h3>
                        <p className="text-brand-green mt-1 font-medium text-sm md:text-base">{t.pricing.soloCard.subtitle}</p>
                    </div>
                    ...
                    <div className="p-4 border-b border-brand-border/50 bg-brand-green/5 relative">
                        <div className="absolute right-3 top-3 md:right-4 md:top-4">
                         ...
                         <div className="flex flex-col">
                             <span className="text-white font-bold text-base md:text-lg">{t.pricing.soloCard.optionA.title}</span>
                             <span className="text-[10px] md:text-xs text-brand-text mb-1 md:mb-2">{t.pricing.soloCard.optionA.desc}</span>
                             <span className="text-xl md:text-2xl font-bold text-brand-green">{t.pricing.soloCard.optionA.price}</span>
                         </div>
                    </div>
                    <div className="p-4 bg-brand-card/30">
                         <div className="flex flex-col">
                             <span className="text-white font-bold text-base md:text-lg">{t.pricing.soloCard.optionB.title}</span>
                             <span className="text-[10px] md:text-xs text-brand-text mb-1 md:mb-2">{t.pricing.soloCard.optionB.desc}</span>
                             <span className="text-xl md:text-2xl font-bold text-white">{t.pricing.soloCard.optionB.price}<span className="text-sm font-normal text-gray-500">{t.pricing.soloCard.optionB.perMonth}</span></span>
                         </div>
                    </div>
                </div>
                ...
                {t.pricing.soloCard.button}
                <ArrowRight className="w-5 h-5" />
                </Link>

                <div className="space-y-3 md:space-y-4">
                {t.pricing.soloCard.features.map((feature, i) => (
            ...
            <div className="bg-[#121212] px-4 md:px-8 py-3 md:py-4 border-t border-brand-border flex items-center justify-center gap-2 text-center">
                <ShieldCheck className="w-4 h-4 text-gray-500 shrink-0" />
                <span className="text-[10px] md:text-xs text-gray-500 font-medium uppercase tracking-wide">{t.pricing.soloCard.commitment}</span>
            </div>
            </div>
        </div>

        <div className="max-w-2xl mx-auto text-center border-t border-brand-border pt-8 md:pt-12">
             <h4 className="text-gray-400 font-medium mb-4 md:mb-6 text-xs md:text-sm uppercase tracking-wider">{t.pricing.paymentMethods}</h4>
